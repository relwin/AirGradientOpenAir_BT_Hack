...
static Configuration configuration(Serial);
 Measurements measurements(configuration);    //RGE
...
enum NetworkOption { UseWifi,
                     UseCellular,
                     UseBLE };        //RGE add BLE option
...
//RGE BLE logging
extern bool ble_loop(void);
extern void ble_setup(void);
extern bool ble_wait_for_connect(long tmo);
...
void setup() {
  ...
  //RGE BLE setup for logging. Connect quickly after reboot.
  ble_setup(); 
  Serial.println("myBLE Wait For Connect");
  if( ble_wait_for_connect(15000) ){
    //RGE bypass wifi setup, force to offline
    Serial.println("myBLE Logging Active, Forcing Offline");
    configuration.setOfflineModeWithoutSave(true);
    connectToNetwork = false;
    networkOption = UseBLE;
  } else {
    Serial.println("myBLE TMO, Normal Operation");
    NimBLEDevice::deinit();  //try this if we're not using BLE now
  }
  ...
  //RGE re-do BLE setup if not using already, lets us log whenever running.
  if( networkOption != UseBLE){
    ble_setup(); 
  }
}
...
void initializeNetwork() {
  ...
  if (agClient->isLastFetchConfigSucceed() == false || configuration.parse(config.c_str(), false) == false) {
    if (ag->isOne()) {
...
static void updateDisplayAndLedBar(void) {
  ...
  } else if (agClient->isLastPostMeasureSucceed() == false && configuration.isPostDataToAirGradient()) {
...
static void updatePm(void) {
  ...
  if (configuration.hasSensorSGP) {
    ...
    if (newPMS1Value && newPMS2Value) {
      ...
      temp = (measurements.getFloat(Measurements::Temperature, 1) + measurements.getFloat(Measurements::Temperature, 2)) / 2.0f;
      hum = (measurements.getFloat(Measurements::Humidity, 1) + measurements.getFloat(Measurements::Humidity, 2)) / 2.0f;
...
void restartIfCeClientIssueOverTwoHours() {
  if (agCeClientProblemDetectedTime > 0 && (MINUTES() - agCeClientProblemDetectedTime) > TIMEOUT_WAIT_FOR_CELLULAR_MODULE_READY) {
    // Give up wait
  ...
  while (1) {
    ...
    if (networkOption == UseWifi) {
      ...
      if (agClient->isClientReady() == false) {
        ...
        if ((MINUTES() - agCeClientProblemDetectedTime) > TIME_TO_START_POWER_CYCLE_CELLULAR_MODULE) {
